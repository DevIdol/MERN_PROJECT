...
const NewsPost = () => {
  ...
  const newsFilter = data.filter((tool) => tool.type === "News");
  const reverseNews = newsFilter
    .slice(0)
    .reverse()
    .filter((tool) => {
  ...
  const sliceNews = reverseNews.slice(0, loadmore);
  const onLoadmore = () => {
  ...
  const isLoadmore = reverseNews.length > loadmore;
        ...
        <Container>
          ...
          {loading ? (
            ...
            <Row>
              ...
              {sliceNews.map((news, index) => {
                return (
                  <Col key={index} md={6}>
                    <Card className={styles.card}>
                      {news.photo && (
                        <Card.Img variant="top" src={PF + news.photo} />
                      )}
                      <Card.Body>
                        <SubTitle cat={news.cat} date={news.createdAt} />
                        <Link to={`/blog/news/${news._id}`}>
